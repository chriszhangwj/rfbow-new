function [ data_Train, data_Test ] = getData_Two_Modes( MODE_CodeBook, param_cb)
...
switch MODE_CodeBook
    case 'rf'
        ...
        param_CodeBOOK.num = param_cb.num;          % Number of trees % intially 50
        param_CodeBOOK.depth = param_cb.depth;        % Depth of each tree
        param_CodeBOOK.splitNum = param_cb.splitNum;     % Number of trials in split function
        param_CodeBOOK.split = param_cb.split;     % Currently support 'information gain' only
        param_CodeBOOK.learner = param_cb.learner;
    ...
    case 'km'
        ...
        desc_sel = single(vl_colsubset(cat(2,desc_tr{:}),1e5)); % Randomly select 100k (1e5) SIFT descriptors for clustering

        disp('K-means clustering...')
        num_centers = 256; 
        [~,C] = kmeans(desc_sel.',num_centers);
        %tic
