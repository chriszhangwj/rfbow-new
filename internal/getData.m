...
%=========================== K-means codebook ===========================%
        ...
        num_centers = 256; % for instance,
        tic
        [~,C] = kmeans(desc_sel.',num_centers,'MaxIter',100);
        %[C,~]=vl_kmeans(desc_sel,num_centers,'MaxNumComparisons',1e8,'MaxNumIterations',100,'algorithm','elkan');
        %[C,~]=vl_ikmeans(uint8(desc_sel),num_centers);
        %[C,~]=vl_hikmeans(uint8(desc_sel),num_centers,num_centers);
        toc
        %profile viewer  
        
%============================== RF codebook =============================%

%         disp('RF codebook clustering...')   
%  
%         param_cb.num = 10;
%         param_cb.depth = 8;
%         param_cb.splitNum = 3;
%         param_cb.split = 'IG';
%         
%         % to be defined desc_rf_tr = [x,y]
%         [] = growTrees(desc_rf_tr,param_cb);
...
%=========================== Encoding images =============================%
        ...
        %%%%%%%
        psix = vl_homkermap(hist_tr, 1, 'kchi2', 'gamma', .5) ;
